...
filenames <- list.files("Benchmarks") %>% lapply(., function(x) { x <- paste0("Benchmarks/", x) }) %>% as.character(.)
...
png(filename = "Benchmarks_summarized.png", width = 1920*3, height = 1080*3, res=300)
#Get plot
